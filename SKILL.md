# TikTok Ad Video Skill (Seedance 2.0 Edition · v2.6)
...
> **迭代版本**：v2.6 (2026.05) —— 统一版本号，精确 TikTok 测试池大小（200-500人），原生感升级为核心创作原则，补充 Fast 模式成本优势。
...
4. **品类匹配多镜头结构**：根据产品品类，从 `viral-hook-patterns.md` 中选择对应的多镜头叙事模板（3-4 个镜头）。
5. **必须植入复播钩子**：每个视频必须包含至少一个复播引导设计（信息差彩蛋、节奏中断、双重结局）。
6. **收藏+社交货币分享优先**：互动引导必须优先设计 **收藏(Save)** 和具备社交货币的 **分享(Share)** 话术，这是 2026 年最高权重信号。
7. **一稿通投，平台微调**：默认生成视频适配多平台，但需针对不同平台的算法特征提供差异化发布指引。
8. **不浪费积分**：任何直接生成视频的操作前，必须先经过 **“钩子图文测试”** 环节。优先推荐用户使用 **Fast 模式**（消耗积分减少 30%-50%）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **🎬 TikTok** | 9:16, 1080x1920, ≤15s | 前 3 秒触发评论欲/分享冲动。必须植入复播彩蛋+收藏+社交货币分享引导。 | ✅ 强支持 | ✅ 高权重 | **小批量测试（200-500人）+ 收藏=分享>评论>点赞，算法月度迭代** |
| **▶️ YouTube Shorts** | 9:16, 1080x1920, ≤60s | 前 2 秒制造新奇反应。首镜头含搜索关键词视觉元素。**避免高划走率**。 | ✅ 支持 | ✅ 支持 | 划走率是关键负向指标，重播率影响推荐 |
...
> **TikTok 分阶段测试分发说明**：新视频发布后，首先进入约 **200-500 人的小批量测试池**（兴趣匹配受众）。测试池的反馈（完播率、收藏、分享）决定能否进入下一级流量池。TikTok 算法更新频率已提升为**月度迭代**，需持续关注官方动态。
...
## 3. 工作流：低成本爆款生成引擎 (v2.6 原生感核心+Fast模式)
...
3. 对照 `viral-hook-patterns.md` (v2.6) 预判该产品最适合的 **1-2 种钩子类型** 及对应的 **多镜头叙事模板**。
4. 预判该产品适合的 **复播彩蛋类型** 及 **社交货币分享类型**（展现品味/展示专业/圈层归属）。
5. **强制启用原生感策略**：除非产品是奢侈品或需要高端精修广告，否则必须使用原生感词汇（素人演员、生活化场景、自然光线、手持自拍感）。
...
### 阶段 3：Seedance 2.0 爆款提示词工程 (v2.6 混写+原生感强化)
**原则**：必须包含 `cinematic-vocabulary.md` (v2.6) 中的 **五维提示词架构**，**推荐使用中英文混写格式**，并**强制融入原生感词汇**。

**提示词构建模板（混写推荐）**：
```
[技术基底（混写）] + [原生感词汇] + [镜头1（混写）] + [镜头2（混写）] + [镜头3（混写）] + [镜头4（混写）] + [复播彩蛋指令] + [音频同步指令] + [去AI味指令] + [平台尾缀]
```
...
### 阶段 5：质量评估与爆款归因
生成视频后，对照 `evaluation-rubric.md` (v2.6) 进行**五维度评分**：
...
- `references/viral-hook-patterns.md` (v2.6) —— 爆款钩子特征库，含品类多镜头模板与社交货币分享体系
- `references/cinematic-vocabulary.md` (v2.6) —— 五维架构词汇表，含中英文混写指南与原生感默认词汇包
- `references/platform-specs.md` (v2.6) —— 全平台最新算法规格，含小批量测试分发与 TikTok SEO 策略
- `references/evaluation-rubric.md` (v2.6) —— 五维度评分表（含导演执行维度）
- `references/product-tracker-template.md` (v2.6) —— 产品追踪与爆款复盘模板（可选）
- `examples/prompt-examples.md` (v2.6) —— 多镜头提示词示例库（含纯英文版与混写版）
- `references/failure-case-library.md` (v2.6) —— 失败案例与修复方案
