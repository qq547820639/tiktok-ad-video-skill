# TikTok 广告视频生成 Skill · 编排者 (v5.0-Orchestrator)
...
**调用方式**:
```
请以"创意生成与诊断师"身份，加载 creative-expert/SKILL.md 及相关reference文件，执行阶段3→4→5→6→6.5的完整闭环。
产品: [用户产品信息]
市场: [目标市场]
用户Hook偏好: [如有，没有则标注"默认选择"]
```

**关键约束**: 创意专家的SKILL文件内已内置**自检熔断机制**，你不需要额外检查其输出完整性。但你需要确认它返回了完整的5个阶段输出。如果某个阶段输出缺失，你必须要求它重新执行。
...
2. **完整性检查**: 每次接收专家输出时，检查是否包含该专家负责的所有阶段输出。缺失则要求重新执行。
...
---
*SKILL.md v5.0-Orchestrator · 编排者不生成，只调度*
