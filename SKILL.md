...
## 二、完整工作流（7 阶段 → 升级为 8 阶段）

> v4.0 新增阶段 5.5：贝叶斯爆款概率预测
...
- **复播彩蛋原理**：最后 1.5 秒设置开放循环（重复一个未完成动作，如锅铲入画但未接触锅面），利用 Zeigarnik 效应强制触发复播，直接推高完播率。
...
- **更新公式**：
  ```
  w_i_new = w_i_old × (CTR_actual / CTR_predicted) × e^(-λ·t)
  ```
...
*SKILL.md v4.0-Adaptive · 2026-04-24 Locked · 加载此文件即视为接受 Bayesian Decision Framework*
```
